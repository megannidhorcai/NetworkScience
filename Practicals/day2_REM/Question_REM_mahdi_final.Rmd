...
# Get the number of unique entries in the column
num_unique_entries <- length(unique(WTCPoliceCalls$source))

# Print the result
print(num_unique_entries)

# Get the number of unique entries in the column
num_unique_entries <- length(unique(WTCPoliceCalls$recipient))
...
g = as.sociomatrix.eventlist(WTCPoliceCalls, 37)
...
```{r}
# Write your code here.
gplot(g, vertex.col = "black", edge.col = "red")
??gplot
...
```{r}
# Write your code here.
 model = rem.dyad(WTCPoliceCalls, n = 37,effects =  c("CovInt"), covar = list(CovInt = WTCPoliceIsICR), hessian = TRUE)
```
...
covRec = exp(model$coef)
covRec

# Relative hazard for an ICR/ICR 
covSnd= exp(model$coef)*exp(model$coef)
...
model1 <- rem.dyad(WTCPoliceCalls, n = 37, effects = c("CovSnd","CovRec"),  covar = list(CovSnd = WTCPoliceIsICR, CovRec = WTCPoliceIsICR), hessian = TRUE)



mean(apply( model$predicted.match,1,all))
mean(apply( model1$predicted.match,1,all))
